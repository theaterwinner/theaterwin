...
def question_list(request):
    ...
    return render(request, 'TheaterWinBook/question_list.html', {"database_list_result": database_list_result})
            ...
            if form.is_valid():  # 폼 검증 메소드
                ...
                if check == 0:
                    new_sequencenum = TheaterWinQuestion.objects.filter(
                        question_groupnum=parent_question_groupnum).aggregate(Max('question_sequencenum_ingroup'))["question_sequencenum_ingroup__max"]
...
def question_detail(request, question_pk):
    ...
    return render(request, 'TheaterWinBook/question_detail.html', {'question_record': target_record, 'form': form, 'is_record_owner':is_record_owner})
    ...
    if target_record.user_name == login_user:
        ...
        if request.method == "POST":
            ...
            if form.is_valid():  # 폼 검증 메소드
                ...
                #   정상적으로 modify로 이동시켜준다.
            form = TheaterWinQuestionForm(instance=target_record)  # forms.py의 PostForm 클래스의 인스턴스
            return render(request, 'TheaterWinBook/question_modify.html', {'form': form, 'record_pk': record_pk})
    else:
            #  유저가 확인이 안되면 에러 페이지로 넘김.
        return render(request, 'TheaterWinBook/error_wronguser.html')
